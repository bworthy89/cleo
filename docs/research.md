...
- **WHAT:** ~~`systemctl enable --now cosyvoice` on <TTS_HOST>. Restart-on-failure, journal logging, auto-start on boot.~~ **Completed in #17:** `cosyvoice.service` ran via `systemctl enable --now cosyvoice` on <TTS_HOST>; service is configured with `Restart=on-failure`, journal logging, and auto-start on boot.
