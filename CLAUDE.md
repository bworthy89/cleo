...
- **Fun-CosyVoice3-0.5B** (self-hosted FastAPI wrapper on the Linux box,
  port 8001, proxied via `<TTS_TUNNEL>/cosy/*`) — **primary TTS**
...
- **F5-TTS** (same Linux box, port 8000, `<TTS_TUNNEL>`) — **fallback
...
- **Shared reference:** `~/f5tts-server/refs/onay-cartesia.wav` + `.txt` is the
  ...
  longer says "Oh-nay").
- **Pangolin tunnel:** `<TTS_TUNNEL>` → port 8000 on this host.
  CosyVoice reached via `<TTS_TUNNEL>/cosy/*` proxy rather than a
  second tunnel rule.
