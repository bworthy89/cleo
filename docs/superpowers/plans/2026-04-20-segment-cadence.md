...
```bash
ssh cleo@<VPS_HOST> 'cd ~/cleo-broadcast && npm run build && pm2 reload cleo-broadcast'
```
...
```bash
ssh cleo@<VPS_HOST> 'curl -s http://localhost:3102/health && echo && pm2 logs cleo-broadcast --lines 10 --nostream'
```
...
3. Watch server logs: `ssh cleo@<VPS_HOST> 'pm2 logs cleo-broadcast'`
