...
- [ ] Smoke-test on staging: either local-device install with `EXPO_PUBLIC_API_URL=https://staging.api.worthymedia.tech` in `.env.local` and bake from the app, OR `curl https://staging.api.worthymedia.tech/health` plus `ssh cleo@187.124.69.95 'pm2 logs cleo-broadcast-staging --lines 50 --nostream'` for any new code paths exercised
- [ ] **Merge `staging` → `main` and push** — prod auto-deploys in ~30s. Watch GH Actions for green health check.
- [ ] PM2 logs sane for ~5 min after prod deploy: `ssh cleo@187.124.69.95 'pm2 logs cleo-broadcast --lines 100 --nostream'` shows no health-check flap, no Sentry spike, no 5xx pattern. (`[skip deploy]` in commit message bypasses auto-deploy — use only for docs-only commits, never code.)
