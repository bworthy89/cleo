...
The production server is a Fastify codebase at `/home/cleo/cleo-api/` on the Hostinger VPS (187.124.69.95). It compiles TypeScript via `npx tsc` — the compiler handles import resolution, so no manual `.js` extension changes are needed in the source `.ts` files.

- [ ] **Step 1: Copy cache.ts to production VPS**

```bash
scp server/src/providers/tts/cache.ts root@187.124.69.95:/home/cleo/cleo-api/src/providers/tts/cache.ts
```
...
```bash
ssh root@187.124.69.95 'cd /home/cleo/cleo-api && npx tsc && fuser -k 3100/tcp; sleep 2; nohup node dist/index.js > server.log 2>&1 &'
```

- [ ] **Step 4: Verify health**

```bash
ssh root@187.124.69.95 'sleep 3; curl -s -o /dev/null -w "%{http_code}" http://localhost:3100/health'
```
...
```bash
ssh root@187.124.69.95 'tail -5 server.log'
```
